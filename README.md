...
- **OBS should always sit on the lower lane in a camera-sync pair (bug)** — current allocation orders camera lanes purely by first-seen chronological order, so OBS lands above or below the paired camera depending on which clip appeared first in the batch. User preference: OBS sits on the lower-numbered channel pair whenever it's one of two cameras in a sync pair, so the visual hierarchy is consistent across projects. Easiest fix is to pre-seed OBS into `camera_channels` before the chronological iteration so it claims a low channel. Decide at implementation time whether this is a strict "OBS first" rule or a more general "screen-capture goes below" rule.
- **Enumerate `.blend` outputs on each cut run** — `vse-validate-markers.py` currently writes `<name>_cut.blend`, overwriting any previous cut from earlier runs of the same project. Suffix subsequent runs with `_2`, `_3`, etc. (e.g. `<name>_cut.blend`, then `<name>_cut_2.blend`, then `<name>_cut_3.blend`) so historical cuts are preserved for comparison or rollback. Decide whether the follow-up `vse-remove-markers.py` next-step hint always points at the latest, or asks.
