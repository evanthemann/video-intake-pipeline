...
- **Automate Blender's render/export settings + run the export** — a new top-level Python script (likely `render-export.py`) that takes the latest `<project>_<N>.blend` from the cut chain and does the render **outside Blender's built-in FFmpeg encoder**, which doesn't expose real CRF/GOP/preset control. Instead: configure a headless PNG image-sequence render via Blender's Python API (RGB, 8-bit, project fps) into a temp frames dir, mixdown the VSE's audio to WAV via the Sequencer's sound mixdown, run `bpy.ops.render.render(animation=True)`, then shell out to **ffmpeg** to mux frames + WAV into the final `<project>_<N>.mp4` — `libx264 -crf 18 -preset slow -pix_fmt yuv420p -c:a aac -b:a 192k`, matching the CRF 18 standard `transcode.py` already holds the rest of the pipeline to. A PNG sequence is lossless and resumable (a crashed/interrupted render loses nothing, unlike an interrupted direct H.264 render), and the external ffmpeg mux gets real encoder control Blender's own exporter can't offer. Keyframe interval 1 (`-g 1`) is a flag, not the default — only worth the file-size cost for a scrub-friendly handoff export (`captions/captions.py`, `after-effects/export-to-ae.py`), not a final delivery copy. Deletes the temp PNG sequence after a successful mux. Same pipeline idioms as the rest of the scripts: ANSI helpers, `sanitize_path` drag-and-drop input, `find_blender` auto-discovery, embedded Blender Python payload as a template string, end-of-script next-step hint pointing at the freshly rendered MP4. Removes the recurring need to drop into Blender's UI just to set render properties and click **Render → Render Animation**.
