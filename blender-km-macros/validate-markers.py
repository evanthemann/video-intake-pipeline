...
# -*- coding: utf-8 -*-
"""
validate-markers.py — First half of the Keyboard Maestro cutting round

Part of the optional `blender-km-macros/` add-on to the Video Intake & Blender
VSE Pipeline, not a numbered pipeline step. The F/u marker convention exists to
drive the Keyboard Maestro cutting macro; without KM you would edit and cut in
Blender directly and never place these markers at all.
...
Usage:
    python3 blender-km-macros/validate-markers.py /path/to/project.blend
    python3 blender-km-macros/validate-markers.py    # prompted to drag-and-drop
    python3 blender-km-macros/validate-markers.py project.blend --blender /path/to/blender

Dependencies:
    blender         — must be on PATH or at the macOS default location
    Keyboard Maestro — for the cutting macro this feeds (macOS only)

Platform: macOS only in practice (the cutting macro it feeds is KM-based).
"""
...
VALIDATE_TEMPLATE = '''\
# AUTO-GENERATED by validate-markers.py — do not edit by hand
import bpy
import os
import sys
...
def resolve_cut_path(blend_file: str) -> str:
    ...
    which remove-markers.py then bumps to `project_<N+1>.blend`.
...
def main():
    parser = argparse.ArgumentParser(
        description="Marker validation step: validate F/u marker pairs, save a "
                    "_cut.blend copy, and print the Keyboard Maestro loop count."
    )
    parser.add_argument(
        "blend_file",
        nargs="?",
        default=None,
        help="Path to the .blend to validate. Omit to be prompted.",
    )
    parser.add_argument(
        "--blender",
        default=None,
    ...
    remover = os.path.join(km_dir, "remove-markers.py")
    loops_str = str(km_loops) if km_loops is not None else "<N>"
    print(f"  1. Open {os.path.basename(cut_path)} in Blender, put the mouse over the")
    print(f"     VSE timeline, then trigger the cutting macro ({loops_str} loop(s)):")
    print(f"       \"{trigger}\" {loops_str}")
    print(f"  2. After the macro finishes, clean up the markers:")
    print(f"       python3 \"{remover}\" \"{cut_path}\"")
    print()
